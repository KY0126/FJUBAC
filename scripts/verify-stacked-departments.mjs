...
if (await stack.locator(".site-department-instagram-pending").count() !== 5 || await stack.getByText("Instagram", { exact: true }).count() !== 5) throw new Error("每張部門卡片應保留一個未連結的 Instagram 聯絡位置。");
if (await stack.getByRole("link", { name: /FJUBAC (Instagram|Threads|LinkedIn)/ }).count() !== 0) throw new Error("未提供部門專屬公開帳號時，卡片不應連至現有官方社群帳號。");
...
if (initialCardMetrics.titleTopGap < 26 || initialCardMetrics.titleTopGap > 40) throw new Error(`焦點卡上框至部門名稱的減半校正間距不正確，目前為 ${initialCardMetrics.titleTopGap.toFixed(1)}px。`);
