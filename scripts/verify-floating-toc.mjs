...
async function verifyDesktopTableOfContents(path, targetLabel, targetId) {
  ...
  const background = await toc.locator(".floating-toc-desktop").evaluate(element => getComputedStyle(element).backgroundColor);
  const alpha = Number(background.match(/rgba?\([^,]+,[^,]+,[^,]+(?:,\s*([\d.]+))?\)/)?.[1] ?? "1");
  if (alpha >= 1) throw new Error(`${path} 桌機章節目錄應採半透明背景，以避免干擾閱讀。`);
...
const mobileBackground = await mobileContents.evaluate(element => getComputedStyle(element).backgroundColor);
const mobileAlpha = Number(mobileBackground.match(/rgba?\([^,]+,[^,]+,[^,]+(?:,\s*([\d.]+))?\)/)?.[1] ?? "1");
if (mobileAlpha >= 1) throw new Error("行動版章節目錄應採半透明背景，以避免干擾閱讀。 ");
