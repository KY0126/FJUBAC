...
import { z } from "zod";
import { auditLogs, eventRegistrations, events, memberships, projectAssignments, projects, resources, users } from "../../drizzle/schema";
import { getDb, getUserClubContext } from "../db";
import { storageGet, storagePut } from "../storage";
import { projectManageProcedure, protectedProcedure, resourceManageProcedure, router } from "../_core/trpc";
...
const projectInput = z.object({ title: z.string().trim().min(2).max(200), description: z.string().trim().max(5000).optional(), departmentId: z.number().int().positive().optional(), startsAt: z.date().optional(), endsAt: z.date().optional(), status: z.enum(["draft", "active", "completed", "archived", "cancelled"]) });
const resourceInput = z.object({ title: z.string().trim().min(2).max(200), description: z.string().trim().max(5000).optional(), fileName: z.string().trim().min(1).max(255), mimeType: z.string().trim().max(120), dataUrl: z.string().min(16).max(14_000_000), visibility: z.enum(["public", "member", "project", "officer"]), projectId: z.number().int().positive().optional(), departmentId: z.number().int().positive().optional(), versionLabel: z.string().trim().max(80).optional() });
...
export const workspaceRouter = router({
  projects: router({
    ...
    create: projectManageProcedure.input(projectInput).mutation(async ({ ctx, input }) => {
      ...
      const result = await db.insert(projects).values({ ...input, description: input.description ?? null, departmentId: input.departmentId ?? null, startsAt: input.startsAt ?? null, endsAt: input.endsAt ?? null, createdByUserId: actor.id });
      await db.insert(auditLogs).values({ actorUserId: actor.id, action: "project.created", targetType: "project", targetId: result[0].insertId, afterData: { status: input.status } });
  ...
  resources: router({
    ...
    upload: resourceManageProcedure.input(resourceInput).mutation(async ({ ctx, input }) => {
      ...
      const result = await db.insert(resources).values({ title: input.title, description: input.description ?? null, storageKey: upload.key, fileName: input.fileName, mimeType: input.mimeType || encodedMime, visibility: input.visibility, projectId: input.projectId ?? null, departmentId: input.departmentId ?? null, versionLabel: input.versionLabel ?? null, createdByUserId: actor.id });
      await db.insert(auditLogs).values({ actorUserId: actor.id, action: "resource.uploaded", targetType: "resource", targetId: result[0].insertId, afterData: { visibility: input.visibility, fileName: input.fileName } });
      return { id: result[0].insertId };
    }),
  }),
  members: router({
