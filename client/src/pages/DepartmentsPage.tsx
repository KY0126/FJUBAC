...
export default function DepartmentsPage() {
  ...
  const departmentSections = useMemo(() => departments.data?.map(department => ({ id: getDepartmentAnchorId(department.name), label: department.name })) ?? [], [departments.data]);
  useEffect(() => {
  ...
  return <main className="service-shell"><PublicSiteHeader section="FIVE DEPARTMENTS" /><FloatingTableOfContents sections={departmentSections} trackScroll={false} /><section className="service-hero"><p className="club-section-number">ORGANIZATION</p><h1>五部門介紹</h1><p>FJUBAC 以五個部門分工處理招募、專案、對外、學術與品牌內容。以下只呈現目前啟用的部門與公開職責，不揭露個人名單或職務任期。</p></section><section className="service-content"><div className="service-ledger"><UsersRound /><div><strong>公開介紹範圍</strong><span>部門分工、學習點與加分項目可公開閱讀；個人資料、任期與內部工作紀錄仍依權限保護。</span></div></div><Reveal>{departments.isLoading ? <div className="service-empty">正在讀取部門介紹…</div> : departments.isError ? <div className="service-empty"><Building2 size={28} /><h2>暫時無法載入部門資料。</h2><p>{departments.error.message || "請稍後重新整理頁面。"}</p><button type="button" className="club-primary" onClick={() => departments.refetch()}>重新載入</button></div> : departments.data?.length ? <div className="department-public-grid">{departments.data.map((department, index) => {
