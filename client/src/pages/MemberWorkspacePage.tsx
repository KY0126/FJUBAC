import { ArrowLeft, Download, FolderKanban, FolderOpen, LockKeyhole, NotebookTabs, UserRound } from "lucide-react";
...
export default function MemberWorkspacePage() {
  ...
  const resourceContent = resources.isLoading ? <div className="workspace-empty">正在整理被授權資源…</div>
    : resources.isError ? <div className="workspace-empty"><NotebookTabs /><h3>暫時無法載入資源。</h3><p>{resources.error.message}</p><button className="club-primary" onClick={() => resources.refetch()}>重新載入</button></div>
      : resources.data?.length ? <div className="resource-list">{resources.data.map(resource => <article className="resource-item" key={resource.id}><NotebookTabs /><div><span className="status-chip">{resource.visibility.toUpperCase()}</span><h3>{resource.title}</h3><p>{resource.description || "尚無描述"}</p><small>{resource.fileName}{resource.versionLabel ? ` · ${resource.versionLabel}` : ""}</small></div><button aria-label={`下載 ${resource.title}`} onClick={() => download.mutate({ resourceId: resource.id })} disabled={download.isPending}><Download size={17} /></button></article>)}</div>
        : <div className="workspace-empty"><NotebookTabs /><h3>目前沒有可存取的資源。</h3><p>資源會由對應部門依公開、社員、專案或幹部範圍發布。</p></div>;
  return <main className="workspace-shell"><header className="workspace-header"><Link href="/" className="back-link"><ArrowLeft size={16} />返回 FJUBAC</Link><div><UserRound size={17} /><span>{user?.name || "社員"}</span></div></header><section className="workspace-hero"><p className="club-section-number">MEMBER DESK</p><h1>我的學習與實作</h1><p>依你的社員、專案生與幹部指派顯示資源與專案。所有範圍皆於後端重新驗證。</p></section><section className="workspace-content"><div className="workspace-section-head"><div><p className="club-section-number">01 / ASSIGNMENTS</p><h2>我的專案</h2></div><span>{projects.data?.length ?? 0} 個有效指派</span></div>{projectContent}<div className="workspace-section-head resource-heading"><div><p className="club-section-number">02 / KNOWLEDGE BASE</p><h2>我的資源</h2></div><span>公開／社員／專案／幹部</span></div>{resourceContent}</section></main>;
}
