...
import { MyTaskSummary } from "@/components/MyTaskSummary";
...
export default function MemberWorkspacePage() {
  ...
  return <main className="workspace-shell"><header className="workspace-header"><Link href="/" className="back-link"><ArrowLeft size={16} />返回 FJUBAC</Link><div><Link href="/me" className="workspace-profile-link"><UserRound size={17} /><span>{user?.name || "社員"}</span></Link></div></header><section className="workspace-hero"><p className="club-section-number">MEMBER DESK</p><h1>我的學習與實作</h1><p>依你的社員、專案生與幹部指派顯示資源與專案。所有範圍皆於後端重新驗證。</p></section><section className="workspace-content"><MyTaskSummary /><div className="workspace-section-head"><div><p className="club-section-number">02 / ASSIGNMENTS</p><h2>我的專案</h2></div><span>{projects.data?.length ?? 0} 個有效指派</span></div>{projectContent}<div className="workspace-section-head resource-heading"><div><p className="club-section-number">03 / KNOWLEDGE BASE</p><h2>我的資源</h2></div><span>公開／社員／專案／幹部</span></div>{resourceContent}</section></main>;
}
