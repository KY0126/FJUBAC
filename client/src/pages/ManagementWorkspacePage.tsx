import { Activity, ArrowLeft, CalendarPlus, FilePlus2, FolderPlus, LockKeyhole, Megaphone, ShieldCheck, UploadCloud, UsersRound } from "lucide-react";
...
export default function ManagementWorkspacePage() {
  ...
  const createEvent = trpc.content.events.create.useMutation({ onSuccess: () => toast.success("活動已建立。"), onError: error => errorToast(error.message) });
  ...
  const submitAnnouncement = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await createAnnouncement.mutateAsync({ title: String(form.get("title")), excerpt: String(form.get("excerpt") || ""), content: String(form.get("content")), visibility: "public", status: "published" }); event.currentTarget.reset(); } catch { /* toast handled by mutation */ }
  };
  const submitEvent = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await createEvent.mutateAsync({ title: String(form.get("title")), summary: String(form.get("summary") || ""), startsAt: new Date(String(form.get("startsAt"))), endsAt: new Date(String(form.get("endsAt"))), registrationDeadlineAt: form.get("deadline") ? new Date(String(form.get("deadline"))) : undefined, location: String(form.get("location") || ""), capacity: Number(form.get("capacity") || 0), visibility, projectId, status: "open" }); event.currentTarget.reset(); } catch { /* toast handled by mutation */ }
  };
  const submitProject = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    try { await createProject.mutateAsync({ title: String(form.get("title")), description: String(form.get("description") || ""), status: "active" }); event.currentTarget.reset(); } catch { /* toast handled by mutation */ }
  };
  ...
  const submitResource = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await uploadResource.mutateAsync({ title: String(form.get("title")), description: String(form.get("description") || ""), fileName: file.name, mimeType: file.type || "application/octet-stream", dataUrl: await readAsDataUrl(file), visibility, projectId }); event.currentTarget.reset(); setFile(null); } catch { /* toast handled by mutation */ }
  };
