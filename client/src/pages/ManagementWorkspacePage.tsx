import { Activity, ArrowLeft, FilePlus2, FolderPlus, ImagePlus, LockKeyhole, Megaphone, ShieldCheck, UploadCloud, UserCog, UsersRound } from "lucide-react";
...
import { DepartmentCarouselSettingsPanel } from "@/components/DepartmentCarouselSettingsPanel";
...
export default function ManagementWorkspacePage() {
  ...
  return <main className="manage-shell">
    ...
    <section className="manage-grid">
      ...
      {isOwner ? <section className="manage-form governance-card"><header><UserCog /><div><h2>帳號管理</h2><p>查詢與管理已建立社團帳號；建立帳號仍須依招生最終核准流程進行。</p></div></header><p className="governance-detail">可編修基本資料、停用／復原帳號或重設啟用流程；所有操作均寫入稽核紀錄，且不提供硬刪除。</p><Link href="/manage/accounts" className="club-secondary">前往帳號管理</Link></section> : null}
      {isOwner ? <DepartmentCarouselSettingsPanel /> : null}
