...
const schoolYearFor = (date: string) => { const value = new Date(`${date}T00:00:00`); const year = value.getFullYear(); return String((value.getMonth() >= 8 ? year : year - 1) - 1911); };
const semesterFor = (date: string) => { const month = new Date(`${date}T00:00:00`).getMonth(); return month >= 8 || month <= 0 ? "第一學期" : "第二學期"; };
...
export default function ResearchArchive() {
  ...
  const records = useMemo(() => reportData.items.filter(item => {
    ...
    return (!keyword.trim() || haystack.includes(keyword.trim().toLocaleLowerCase())) &&
      ...
      (!selectedSemester || semesterFor(item.date) === selectedSemester);
  }), [keyword, selectedTag, selectedYear, selectedSemester]);
  ...
  return <div className="service-shell teaching-records-page">
    ...
    <main>
      ...
      <section className="service-content teaching-records-content" aria-label="社課教學紀錄">
        <div className="teaching-filter-panel"><div className="teaching-search"><Search size={18} /><label className="sr-only" htmlFor="teaching-search">搜尋教學紀錄</label><input id="teaching-search" value={keyword} onChange={event => setKeyword(event.target.value)} placeholder="搜尋主題、方法或文章名稱" /></div><div className="teaching-select-row"><label>學年度<select value={selectedYear} onChange={event => { setSelectedYear(event.target.value); setSelectedSemester(""); }}><option value="全部">全部</option>{years.filter(year => year !== "全部").map(year => <option key={year} value={year}>{year} 學年度</option>)}</select></label><label>學期<select value={selectedSemester} disabled={selectedYear === "全部"} onChange={event => setSelectedSemester(event.target.value)}><option value="">{selectedYear === "全部" ? "請先選擇學年" : "全部學期"}</option><option value="第一學期">第一學期</option><option value="第二學期">第二學期</option></select></label></div><div className="teaching-filter-row"><span><Tags size={15} />標籤</span><button type="button" aria-pressed={selectedTag === "全部"} onClick={() => setSelectedTag("全部")}>全部</button>{tags.map(tag => <button type="button" key={tag} aria-pressed={selectedTag === tag} onClick={() => setSelectedTag(tag)}>{tag}</button>)}</div></div>
        <p className="teaching-result-count">顯示 <strong>{records.length}</strong> / {reportData.items.length} 則已核對內容</p>
        {records.length ? <div className="teaching-record-grid">{records.map(item => <article className="teaching-record-card" key={item.id}><div className="teaching-card-visual" aria-hidden="true"><BookOpenText size={42} /></div><div className="teaching-card-body"><span className="teaching-year">{schoolYearFor(item.date)} 學年度 · {semesterFor(item.date)} · {item.type}</span><span className="teaching-tag">{tagFor(item.title)}</span><h2>{item.title}</h2><p>{item.theme}主題的公開內容紀錄，可由原始公開頁核對發布資訊。</p><a href={item.source} target="_blank" rel="noreferrer">查看公開來源 <ExternalLink size={15} /></a></div></article>)}</div> : <div className="service-empty"><Search size={28} /><h2>找不到符合條件的教學紀錄</h2><p>請調整搜尋文字、學年、學期或主題標籤。</p></div>}
      </section>
    </main>
    <PublicSiteFooter />
  </div>;
