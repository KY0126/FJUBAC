...
export default function LearningMapPage() {
  return <main className="service-shell"><PublicSiteHeader section="LEARNING MAP" /><FloatingTableOfContents sections={[...JOURNEY.map(({ step, title }) => ({ id: `learning-stage-${step}`, label: title })), { id: "learning-scope-note", label: "公開範圍說明" }]} trackScroll={false} /><section className="service-hero"><p className="club-section-number">OPEN LEARNING FRAMEWORK</p><h1>學習地圖</h1><p>這是一份依 FJUBAC 已核對公開內容整理的探索框架，而不是保證開設的課程表、固定時程或個人成效承諾。實際活動與參與方式仍以正式公告為準。</p><a className="scroll-cue" href="#learning-journey"><span>向下查看探索架構</span><ChevronDown size={16} aria-hidden="true" /></a></section><section className="service-content"><div className="service-ledger"><UsersRound /><div><strong>如何閱讀</strong><span>可把四個階段當作理解與規劃參與的參考，不代表每位社員都會經歷相同路徑。</span></div></div><Reveal><div id="learning-journey" className="learning-journey">{JOURNEY.map(({ step, title, description, Icon }, index) => <article key={step} id={`learning-stage-${step}`} className="learning-stage"><span>{step}</span><Icon size={24} /><h2>{title}</h2><p>{description}</p>{index < JOURNEY.length - 1 ? <ArrowRight className="learning-stage-arrow" aria-hidden="true" /> : null}</article>)}</div></Reveal><Reveal><section id="learning-scope-note" className="learning-note"><div><p className="club-section-number">SCOPE NOTE</p><h2>公開脈絡，而非虛構的成果清單</h2><p>此頁僅將可核對的公開主題歸納為學習方向。若尚未有公告、活動、資源或經同意公開的成果，網站會清楚顯示空白狀態，而不以範例或假資料補足。</p></div><div className="learning-note-actions"><Link href="/announcements" className="club-primary">查看正式公告</Link><Link href="/research" className="club-secondary">查看公開研究範圍</Link></div></section></Reveal></section><PublicSiteFooter /></main>;
}
