...
import { BriefcaseBusiness, Building2, Coffee, FileText, GraduationCap, LockKeyhole } from "lucide-react";
...
export default function LearningMapPage() {
  ...
  return <main className="teaching-records-page"><section className="container teaching-records-content py-12 sm:py-16"><section className="max-w-3xl"><p className="text-sm font-semibold tracking-[.18em] text-blue-700">LEARNING & CAREER MAP</p><h1 className="mt-3 text-3xl font-bold text-slate-950 sm:text-5xl">學習與職涯地圖</h1><p className="mt-5 text-base leading-8 text-slate-700">依四大培育範疇整理的受權限保護資源。頁面只會顯示您依資源可見範圍與有效專案身分可讀取的真實內容。</p></section><Tabs value={category} onValueChange={value => setCategory(value as typeof category)} className="mt-9"><TabsList className="h-auto w-full flex-wrap justify-start gap-2 bg-transparent p-0">{categories.map(item => <TabsTrigger key={item.key} value={item.key} className="data-[state=active]:bg-blue-800 data-[state=active]:text-white">{item.title}</TabsTrigger>)}</TabsList></Tabs><section className="mt-6"><header className="mb-5 flex gap-3"><Icon className="mt-1 h-6 w-6 text-blue-700" /><div><h2 className="text-2xl font-bold text-slate-950">{selected.title}</h2><p className="mt-1 text-slate-700">{selected.description}</p></div></header>{query.isLoading ? <p className="text-slate-700">正在載入資源…</p> : (query.data?.length ?? 0) === 0 ? <Card className="border-blue-100"><CardContent className="py-8 text-slate-700">此分類目前沒有可依您的授權範圍顯示的正式資源。系統不會建立或展示虛構題庫、企業紀錄、照片、心得、影音或職涯資料。</CardContent></Card> : <div className="teaching-record-grid learning-map-card-grid">{query.data?.map(({ mapping, resource }) => <article key={mapping.id} className="teaching-record-card"><div className="teaching-card-visual learning-map-card-visual"><FileText size={35} aria-hidden="true" /><span>{selected.title}</span></div><div className="teaching-card-body"><div className="flex items-start justify-between gap-3"><span className="teaching-tag">{resource.visibility === "project" ? "專案限定" : resource.visibility === "officer" ? "幹部限定" : resource.visibility === "member" ? "社員資源" : "已公開資源"}</span><Badge variant="secondary">資源</Badge></div><h3>{resource.title}</h3><p>{resource.description || "尚未提供說明。"}</p><small className="teaching-year">檔案：{resource.fileName}{resource.versionLabel ? ` · ${resource.versionLabel}` : ""}</small></div></article>)}</div>}</section></section></main>;
}
